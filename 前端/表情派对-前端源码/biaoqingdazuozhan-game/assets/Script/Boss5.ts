...
@ccclass
export default class Boss5 extends cc.Component {

    @property({ type: cc.Node, tooltip: "血条" })
    xueNode: cc.Node = null;

    @property({ type: cc.Node, tooltip: "灰血条" })
    huiBgNode: cc.Node = null;

    @property({ type: cc.Node, tooltip: "灰血条" })
    huiNode: cc.Node = null;

    @property({ type: cc.Node, tooltip: "护盾" })
    huDunNode: cc.Node = null;
    ...
    onEnable () {
        this.ysBool = false;
        this.huDunNode.active = true;
        this.huiBgNode.active = true;
        this.dieNode.active = false;
    }

    sethp(hp:any,shp:any,pos:any){
        this.allHP = hp;
        this.banHP = GameFidhtMessage.Fight_Enemy_Small_Monster[5].minion_blood;
        if(this.banHP == null || this.banHP == undefined ){
            this.banHP = hp*1.3;
        }
        // this.banHP = Math.floor(hp*1.3)
    ...
    enemyHP(){
        ...
        if(this.node.hp > this.allHP){
            this.huDunNode.active = true;
            let ki = this.node.hp - this.allHP;
            this.huiNode.width = (ki/this.banHP)*130
            this.huiBgNode.active = true;
        }else{
            this.huDunNode.active = false;
            this.huiNode.active = false;
            this.xueNode.width = (this.node.hp/this.allHP)*130
            this.huiBgNode.active = false;
        }
