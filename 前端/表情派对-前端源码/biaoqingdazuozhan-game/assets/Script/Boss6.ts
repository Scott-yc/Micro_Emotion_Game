...
export default class Boss2 extends cc.Component {
    ...
    @property({ type: cc.Prefab, tooltip: "蛋蛋" })
    danPre: cc.Prefab = null;
    ...
    @property({ type: cc.Node, tooltip: "X" })
    xMcNode: cc.Node = null;
    ...
    onEnable () {
        ...
        this.xMcNode.active = false;
        let t = GameFidhtMessage.Fight_Enemy_Elite6_Interval_Time
        // t = 10 //测试
        this.schedule(this.xiadanShow,t)
    }
    
    sethp(hp:any,shp:any,pos:any){
        this.allHP = hp;
        this.node.xiadan = false;
    ...
    enemyHP(){
        ...
        if(this.node.hp <= 0){
            ...
            cc.tween(this.node)
                ...
                .to(0.2, { opacity: 0 })
                .call(() => {
                    this.unschedule(this.xiadanShow)
                    // this.node.active = false
    ...
    xiadanShow(){
        ...
        this.xMcNode.active = true;
        this.node.xiadan = true;
        let t = GameFidhtMessage.Fight_Enemy_Elite6_Stop_Time;
        // t = 1.5 //测试
        cc.tween(this.node)
            .delay(t)
            .to(0.2, { scale: 0.8 })
            ...
            .call(() => {
                this.xMcNode.active = false;
                this.node.xiadan = false;
                let dan = cc.instantiate(this.danPre);
                // let view = bian.getComponent("bianbian");
                // dan.zIndex = 0;
                // dan.setSiblingIndex(0)
                // console.log(this.node.parent.parent.children[1])
                dan.setPosition(this.node.getPosition())
                this.node.parent.parent.children[1].addChild(dan)
